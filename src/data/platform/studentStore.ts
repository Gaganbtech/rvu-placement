...
class PlatformStore {
  ...
  public applyToOpportunity(opportunityId: string): { success: boolean; applicationId?: string; message: string } {
    ...
    const appIndex = this.applications.length + 1;
    const newAppId = `DEMO-APP-000${appIndex}`;
    const now = new Date();
    const dateFormatted = `${now.getDate()} Sep 2026, ${now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
  ...
  public scheduleRecruiterAssessment(assessmentData: Omit<RecruiterAssessment, 'id' | 'companyId'>): RecruiterAssessment {
    ...
    newAssessment.candidates.forEach(cand => {
      this.notifications = [
        {
          id: `NOTIF-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
          studentId: cand.studentId,
          title: `Assessment Scheduled: ${newAssessment.title}`,
          message: `${company?.name || 'Recruiter'} has scheduled a proctored assessment (${newAssessment.platform}). Window: ${newAssessment.startDate} to ${newAssessment.endDate}.`,
          category: 'INTERVIEW',
          origin: 'RECRUITER',
          timestamp: 'Just now',
          isRead: false,
          actionRoute: `/student/applications/${cand.applicationId}`
        },
        ...this.notifications
      ];
    });

    this.logAction('Assessment Published', 'Assessment', id, `Recruiter scheduled ${newAssessment.title} on ${newAssessment.platform}`);
    this.notify();
    return newAssessment;
  }

  public evaluateAssessmentCandidate(assessmentId: string, candidateId: string, score: number, feedback: string) {
    this.assessments = this.assessments.map(assess => {
      if (assess.id === assessmentId) {
        const updatedCandidates = assess.candidates.map(cand => {
          if (cand.studentId === candidateId) {
            return {
              ...cand,
              score,
              feedback,
              status: 'EVALUATED' as const,
              evaluatedBy: this.getActiveRecruiter()?.name || 'Technical Panel'
            };
          }
          return cand;
        });
        return { ...assess, candidates: updatedCandidates };
      }
      return assess;
    });

    this.logAction('Assessment Evaluated', 'Assessment', assessmentId, `Candidate ${candidateId} evaluated with score ${score}/100`);
    this.notify();
  }
